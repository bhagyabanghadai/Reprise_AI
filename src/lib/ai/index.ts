...
export class AIService {
  ...
  /**
   * Generate a progression plan for key lifts
   */
  async generateProgressionPlan(
    userId: string,
    exercises: Exercise[],
    currentMaxes: { [exerciseId: number]: number },
    workoutHistory: any[]
  ): Promise<{
    progressionPlans: {
      exerciseId: number;
      ...
      timeframe: string;
      planType: string;
      weeklyProgression: { week: number; weight: number; sets: number; reps: number }[];
    }[];
  }> {
    try {
      const exercisesStr = JSON.stringify(exercises);
      const currentMaxesStr = JSON.stringify(currentMaxes);
      const workoutHistoryStr = JSON.stringify(workoutHistory);
      
      const prompt = `
        As an expert AI strength coach, create a progression plan for this user's key lifts.
        ...
        Create a detailed progression plan for up to 3 main lifts that includes:
        1. Current max
        2. Target max
        3. Realistic timeframe to reach the target
        4. Type of progression (linear, wave, block periodization)
        5. Week-by-week weight, sets, and reps
          ...
          "progressionPlans": [
            {
              ...
              "targetMax": 245,
              "timeframe": "8 weeks",
              "planType": "Linear Periodization",
              "weeklyProgression": [
                { "week": 1, "weight": 185, "sets": 4, "reps": 6 },
                ...
              ]
            },
            ...
      ...
      const completion = await this.client.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert strength coach who specializes in progression planning for strength athletes. You always respond with valid JSON objects only."
          },
          {
            role: "user",
            content: prompt
      ...
      // Return a default progression plan for one exercise in case of API failure
      return {
        progressionPlans: [
          {
            exerciseId: 1,
            exerciseName: "Bench Press",
            currentMax: Object.keys(currentMaxes).length > 0 ? currentMaxes[1] || 135 : 135,
            targetMax: Object.keys(currentMaxes).length > 0 ? Math.round((currentMaxes[1] || 135) * 1.1) : 150,
            timeframe: "8 weeks",
            planType: "Linear Progression",
            weeklyProgression: [
              { week: 1, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 0.75), sets: 3, reps: 8 },
              { week: 2, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 0.8), sets: 3, reps: 6 },
              { week: 3, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 0.85), sets: 4, reps: 5 },
              { week: 4, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 0.7), sets: 3, reps: 10 },
              { week: 5, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 0.85), sets: 3, reps: 6 },
              { week: 6, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 0.9), sets: 4, reps: 4 },
              { week: 7, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 0.95), sets: 3, reps: 3 },
              { week: 8, weight: Math.round((Object.keys(currentMaxes).length > 0 ? (currentMaxes[1] || 135) : 135) * 1.0), sets: 2, reps: 2 }
            ]
          }
        ]
      };
...
// Re-export workout recommendation functions for backward compatibility
export { 
  generateWorkoutPlan, 
  analyzeWorkout 
